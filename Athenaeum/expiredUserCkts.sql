--metadb:function expiredUserCkts

DROP FUNCTION IF EXISTS expiredUserCkts;

CREATE FUNCTION expiredUserCkts()
RETURNS TABLE(
    user_last_name text,
...
where
	...
	'status',
	'name') = 'Open'  and
    jsonb_extract_path_text(ug.jsonb, 'expirationDate')::timestamptz <= (CURRENT_DATE - INTERVAL '14 days')
order by
    jsonb_extract_path_text(ug.jsonb,
	'personal',
	'lastName'),
